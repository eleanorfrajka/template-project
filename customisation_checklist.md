...
```bash
mv src/template_project/ src/your_package_name/
```
...
- `[tool.setuptools_scm] write_to` — path inside your renamed package directory (under `src/`)
...
### `src/template_project/logger.py` (after renaming)
...
- `src/template_project/readers/rapid.py` — RAPID-specific reader; remove if not needed
